...
final class AppDelegate: NSObject, NSApplicationDelegate {
    ...
    private let toggleMenuItem = NSMenuItem(title: "启用小键盘", action: #selector(toggleState(_:)), keyEquivalent: "")
    ...
    private func makeSettingsWindow() -> NSWindow {
        ...
        let button = NSButton(title: "设置切换键", target: self, action: #selector(beginCaptureShortcut(_:)))
    ...
    private func isHotKeyEvent(keyCode: CGKeyCode, flags: CGEventFlags) -> Bool {
        return keyCode == hotKey.keyCode && flags.intersection([.maskCommand, .maskAlternate, .maskControl, .maskShift]) == hotKey.modifiers
    }

    private func shouldRemap(keyCode: CGKeyCode, flags: CGEventFlags) -> Bool {
    ...
    private func sendRemapped(_ keyCode: CGKeyCode) {
