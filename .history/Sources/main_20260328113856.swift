...
class AppDelegate: NSObject, NSApplicationDelegate {
    ...
    func setupStatusBar() {
        ...
        let menu = NSMenu()
        menu.addItem(NSMenuItem(title: isEnabled ? "关闭小键盘" : "开启小键盘", action: #selector(toggleKeypad), keyEquivalent: ""))
    ...
    func mapKeyToNumber(keyCode: Int64, originalEvent: CGEvent) -> Unmanaged<CGEvent>? {
        ...
        if let mappedChar = keyMap[keyCode] {
            let newEvent = CGEvent(keyboardEventSource: nil, virtualKey: 0, keyDown: true)
    ...
    @objc func showSettings() {
        ...
        let cmdCheckbox = NSButton(checkboxWithTitle: "Command", target: self, action: #selector(modifierChanged))
        cmdCheckbox.state = (hotKeyModifiers & UInt32(cmdKey)) != 0 ? .on : .off
        cmdCheckbox.frame = NSRect(x: 120, y: 210, width: 100, height: 24)
        cmdCheckbox.identifier = NSUserInterfaceItemIdentifier("cmdCheckbox")
        contentView.addSubview(cmdCheckbox)
        
        let optCheckbox = NSButton(checkboxWithTitle: "Option", target: self, action: #selector(modifierChanged))
        optCheckbox.state = (hotKeyModifiers & UInt32(optionKey)) != 0 ? .on : .off
        optCheckbox.frame = NSRect(x: 230, y: 210, width: 100, height: 24)
        optCheckbox.identifier = NSUserInterfaceItemIdentifier("optCheckbox")
        ...
        recordButton.identifier = NSUserInterfaceItemIdentifier("recordButton")
        contentView.addSubview(recordButton)
        
        let infoLabel = NSTextField(wrappingLabelWithString: "键位映射说明:\nJ → 1\nK → 2\nL → 3\nU → 4\nI → 5\nO → 6\nM → 0\n逗号 → 00")
    ...
    @objc func modifierChanged() {
        guard let window = settingsWindow,
              ...
              let cmdCheckbox = contentView.view(withIdentifier: NSUserInterfaceItemIdentifier("cmdCheckbox")) as? NSButton,
              let optCheckbox = contentView.view(withIdentifier: NSUserInterfaceItemIdentifier("optCheckbox")) as? NSButton else {
            return
        }
    ...
    @objc func recordHotKey() {
        guard let window = settingsWindow,
              let contentView = window.contentView,
              let recordButton = contentView.view(withIdentifier: NSUserInterfaceItemIdentifier("recordButton")) as? NSButton else {
            return
        }
    ...
    func updateKeyField() {
        guard let window = settingsWindow,
              let contentView = window.contentView,
              let keyField = contentView.view(withIdentifier: NSUserInterfaceItemIdentifier("keyField")) as? NSTextField else {
            return
        }
