...
final class AppDelegate: NSObject, NSApplicationDelegate {
    ...
    private weak var recHint: NSTextField?  // 录制提示标签
    ...
    func onEvent(proxy: CGEventTapProxy, type: CGEventType, event: CGEvent) -> Unmanaged<CGEvent>? {
        ...
        // 录制中：tap 层直接吞掉，让 global monitor 来捕获
        if isRecording {
            return nil
        }
    ...
    private func handleRecEvent(_ ev: NSEvent) {
        ...
        hotMods = cgMods
        hotCode = UInt32(kc)
        log("[REC] 录制成功 mods=\(cgMods) code=\(kc) => \(hotkeyStr())")
        saveSettings()
        DispatchQueue.main.async { self.finishRec() }
    }
