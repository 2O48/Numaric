...
class AppDelegate: NSObject, NSApplicationDelegate {
    ...
    func checkAccessibilityPermission() {
        ...
        print("Accessibility permission: \(isTrusted)")
        if !isTrusted {
    ...
    func setupEventTap() {
        ...
        print("Default hotkey set to: \(hotKeyToString())")
        ...
        let eventTap = CGEvent.tapCreate(
            ...
            callback: { (proxy, type, event, refcon) -> Unmanaged<CGEvent>? in
                ...
                if cmdPressed { modifiers |= (1 << kCGEventFlagMaskCommand) }
                if optPressed { modifiers |= (1 << kCGEventFlagMaskAlternate) }
                if shiftPressed { modifiers |= (1 << kCGEventFlagMaskShift) }
                if controlPressed { modifiers |= (1 << kCGEventFlagMaskControl) }
    ...
    func stopRecording() {
        ...
        // 禁用录制事件监听器
        if let recordEventTap = recordEventTap {
            CGEvent.tapEnable(tap: recordEventTap, enable: false)
            self.recordEventTap = nil
            print("Record hotkey event tap disabled")
        }
    ...
    func hotKeyToString() -> String {
        ...
        if hotKeyModifiers & (1 << kCGEventFlagMaskCommand) != 0 {
            parts.append("⌘")
        }
        if hotKeyModifiers & (1 << kCGEventFlagMaskAlternate) != 0 {
            parts.append("⌥")
        }
        if hotKeyModifiers & (1 << kCGEventFlagMaskShift) != 0 {
            parts.append("⇧")
        }
        if hotKeyModifiers & (1 << kCGEventFlagMaskControl) != 0 {
            parts.append("⌃")
        }
    ...
    func loadSettings() {
        ...
        if let modifiers = defaults.object(forKey: "hotKeyModifiers") as? UInt32 {
            ...
            // 设置默认快捷键为 Cmd+Opt+K
            hotKeyModifiers = (1 << kCGEventFlagMaskCommand) | (1 << kCGEventFlagMaskAlternate)
