    ...
    @objc func recordHotKey() {
        ...
        recordingEventMonitor = NSEvent.addLocalMonitorForEvents(matching: .keyDown) { [weak self] event in
            ...
            if self.isRecording {
                ...
                print(
                    "Record hotkey - Key pressed: \(keyCode), modifiers: \(modifiers.rawValue)"
                )
                
                // 提取修饰键 - 使用 NSEvent.ModifierFlags 检查
                let cmdPressed = modifiers.contains(.command)
                let optPressed = modifiers.contains(.option)
                ...
                // 重要：使用 Carbon 常量存储，与 setupEventTap 中的检测逻辑保持一致
