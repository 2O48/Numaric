...
class AppDelegate: NSObject, NSApplicationDelegate {
    ...
    @objc func recordHotKey() {
        ...
        // 使用 CGEvent.tapCreate 来录制快捷键，确保与检测逻辑使用相同的修饰键值
        let eventMask = (1 << CGEventType.keyDown.rawValue)
        ...
        let eventTap = CGEvent.tapCreate(
            ...
            callback: { (proxy, type, event, refcon) -> Unmanaged<CGEvent>? in
                ...
                if appDelegate.isRecording && type == .keyDown {
                    let keyCode = event.getIntegerValueField(.keyboardEventKeycode)
                    let flags = event.flags
                    
                    // 提取修饰键 - 使用与 setupEventTap 相同的逻辑
                    let cmdPressed = flags.contains(.maskCommand)
                    let optPressed = flags.contains(.maskAlternate)
                    let shiftPressed = flags.contains(.maskShift)
                    let controlPressed = flags.contains(.maskControl)
                    ...
                    print(
                        "Record hotkey - Key pressed: \(keyCode)"
                    )
                    print(
                        "Modifiers check - Command: \(cmdPressed), Option: \(optPressed), Shift: \(shiftPressed), Control: \(controlPressed)"
                    )
                    
                    var modifiersValue = UInt32(0)
                    if cmdPressed { modifiersValue |= UInt32(cmdKey) }
                    if optPressed { modifiersValue |= UInt32(optionKey) }
                    if shiftPressed { modifiersValue |= UInt32(shiftKey) }
                    if controlPressed { modifiersValue |= UInt32(controlKey) }
                    ...
                    if modifiersValue > 0 && keyCode >= 0 && keyCode <= 127 {
                        appDelegate.hotKeyModifiers = modifiersValue
                        appDelegate.hotKeyKeyCode = UInt32(keyCode)
                        
                        print(
                            "Recorded hotkey: modifiers=\(modifiersValue), keyCode=\(keyCode), hotkey=\(appDelegate.hotKeyToString())"
                        )
                        ...
                        DispatchQueue.main.async {
                            ...
                            if let button = appDelegate.recordButton {
                                button.title = "录制"
        ...
        if let eventTap = eventTap {
            let runLoopSource = CFMachPortCreateRunLoopSource(kCFAllocatorDefault, eventTap, 0)
            self.recordingEventTap = eventTap
            self.recordingRunLoopSource = runLoopSource
            CFRunLoopAddSource(CFRunLoopGetCurrent(), runLoopSource, .commonModes)
            CGEvent.tapEnable(tap: eventTap, enable: true)
            ...
            DispatchQueue.main.asyncAfter(deadline: .now() + 5) { [weak self] in
                if self?.isRecording == true {
                    ...
                    // 禁用事件监听器
                    if let tap = self?.recordingEventTap {
