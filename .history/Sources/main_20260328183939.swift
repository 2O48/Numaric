...
class AppDelegate: NSObject, NSApplicationDelegate {
    ...
    func setupEventTap() {
        ...
        let eventTap = CGEvent.tapCreate(
            ...
            callback: { (proxy, type, event, refcon) -> Unmanaged<CGEvent>? in
                ...
                // 调试信息：打印详细的修饰键值
                let savedModifiers = appDelegate.hotKeyModifiers
                let savedKeyCode = appDelegate.hotKeyKeyCode
                print(
                    "=== Hotkey Check ==="
                )
                print(
                    "Current modifiers: \(modifiers) (cmd:\(cmdPressed), opt:\(optPressed), shift:\(shiftPressed), ctrl:\(controlPressed))"
                )
                print(
                    "Saved modifiers: \(savedModifiers), Saved keyCode: \(savedKeyCode)"
                )
                print(
                    "Current keyCode: \(keyCode)"
                )
                print(
                    "Modifiers match: \(modifiers == savedModifiers)"
                )
                print(
                    "KeyCode match: \(keyCode == Int32(savedKeyCode))"
                )
    ...
    @objc func recordHotKey() {
        ...
        if let monitor = recordingEventMonitor {
            ...
            print("Removed previous recording monitor")
        }
        ...
        recordingEventMonitor = NSEvent.addLocalMonitorForEvents(matching: .keyDown) { [weak self] event in
            ...
            if self.isRecording {
                ...
                // 获取原始修饰键值用于调试
                let rawModifiers = modifiers.rawValue
                print(
                    "Record hotkey - Key pressed: \(keyCode), raw modifiers: \(rawModifiers)"
                )
                
                // 直接使用 NSEvent.ModifierFlags 的 rawValue 来检测修饰键
                let rawModifiers = modifiers.rawValue
                print(
                    "Raw modifiers value: \(rawModifiers)"
                )
                
                // NSEvent.ModifierFlags 的位掩码值：
                // command: 1 << 20 = 1048576
                // option: 1 << 19 = 524288
                // shift: 1 << 18 = 262144
                // control: 1 << 17 = 131072
                
                let hasCommand = (rawModifiers & (1 << 20)) != 0
                let hasOption = (rawModifiers & (1 << 19)) != 0
                let hasShift = (rawModifiers & (1 << 18)) != 0
                let hasControl = (rawModifiers & (1 << 17)) != 0
                
                print(
                    "Modifiers check - Command: \(hasCommand), Option: \(hasOption), Shift: \(hasShift), Control: \(hasControl)"
                )
                ...
                // optionKey = 1 << 9 = 512
                // shiftKey = 1 << 10 = 1024
                // controlKey = 1 << 11 = 2048
                var modifiersValue = UInt32(0)
                
                if hasCommand { modifiersValue |= 256 }  // cmdKey
                if hasOption { modifiersValue |= 512 }   // optionKey
                if hasShift { modifiersValue |= 1024 }   // shiftKey
                if hasControl { modifiersValue |= 2048 } // controlKey
                
                print(
                    "Built modifiersValue: \(modifiersValue) (cmd:256, opt:512, shift:1024, ctrl:2048)"
                )
                
                // 只记录带有修饰键的组合键
                if modifiersValue > 0 && keyCode >= 0 && keyCode <= 127 {
                    self.hotKeyModifiers = modifiersValue
                    self.hotKeyKeyCode = UInt32(keyCode)
                    
                    print(
                        "Recorded hotkey: modifiers=\(modifiersValue), keyCode=\(keyCode), hotkey=\(self.hotKeyToString())"
                    )
                    ...
                    if let monitor = self.recordingEventMonitor {
                        ...
                        print("Removed recording monitor")
                    }
        ...
        print("Recording monitor created")
        ...
        DispatchQueue.main.asyncAfter(deadline: .now() + 5) { [weak self] in
            if self?.isRecording == true {
                ...
                if let monitor = self?.recordingEventMonitor {
                    NSEvent.removeMonitor(monitor)
                    self?.recordingEventMonitor = nil
                    print("Removed recording monitor (timeout)")
                }
    ...
    func stopRecording() {
        ...
        // 移除录制事件监听器
        if let runLoopSource = recordingRunLoopSource {
            CFRunLoopRemoveSource(CFRunLoopGetCurrent(), runLoopSource, .commonModes)
            recordingRunLoopSource = nil
        }
        if let eventTap = recordingEventTap {
            CGEvent.tapEnable(tap: eventTap, enable: false)
            recordingEventTap = nil
        }
        print("Recording event tap disabled")
