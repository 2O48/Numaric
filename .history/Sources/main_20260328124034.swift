...
class AppDelegate: NSObject, NSApplicationDelegate {
    ...
    func applicationDidFinishLaunching(_ notification: Notification) {
        ...
        // 自动启用小键盘模式进行测试
        print("Auto-enabling keypad mode for testing")
        isEnabled = true
        enableEventTap()
        updateStatusBarMenu()
    ...
    func testKeyMapping() {
        ...
        // 测试J键映射到1
        let testEvent = CGEvent(keyboardEventSource: nil, virtualKey: 38, keyDown: true)
        if let event = testEvent {
            let mappedEvent = mapKeyToNumber(keyCode: 38, originalEvent: event)
            print("Test J key: mappedEvent = \(mappedEvent)")
        }

        // 测试K键映射到2
        let testEvent2 = CGEvent(keyboardEventSource: nil, virtualKey: 40, keyDown: true)
        if let event = testEvent2 {
            let mappedEvent = mapKeyToNumber(keyCode: 40, originalEvent: event)
            print("Test K key: mappedEvent = \(mappedEvent)")
        }

        // 测试L键映射到3
        let testEvent3 = CGEvent(keyboardEventSource: nil, virtualKey: 37, keyDown: true)
        if let event = testEvent3 {
            let mappedEvent = mapKeyToNumber(keyCode: 37, originalEvent: event)
            print("Test L key: mappedEvent = \(mappedEvent)")
        }

        print("Key mapping test completed")
    }
    ...
    func setupStatusBar() {
        ...
        menu.addItem(
            NSMenuItem(
                ...
                keyEquivalent: ""))
        menu.addItem(NSMenuItem(title: "测试切换", action: #selector(testToggle), keyEquivalent: ""))
    ...
    @objc func testToggle() {
        print("Test toggle button pressed")
        toggleKeypad()
    }
    ...
    func setupGlobalHotKey() {
        ...
        let eventTap = CGEvent.tapCreate(
            ...
            callback: { (proxy, type, event, refcon) -> Unmanaged<CGEvent>? in
                ...
                let cmdPressed = (flags.rawValue & UInt64(cmdKey)) != 0
                let optPressed = (flags.rawValue & UInt64(optionKey)) != 0
    ...
    func mapKeyToNumber(keyCode: Int64, originalEvent: CGEvent) -> Unmanaged<CGEvent>? {
        ...
        if let mappedChar = keyMap[keyCode] {
            ...
            // 直接修改原始事件的keyCode
            let firstChar = mappedChar.first!
            let newKeyCode = charToKeyCode(firstChar)
            print("Changing keyCode from \(keyCode) to \(newKeyCode)")

            // 创建一个新的事件，使用新的keyCode
            let eventSource = CGEventSource(stateID: .hidSystemState)
            let newEvent = CGEvent(
                keyboardEventSource: eventSource, virtualKey: newKeyCode, keyDown: true)

            return newEvent?.toUnmanaged()
        }
