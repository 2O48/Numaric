    ...
    @objc func recordHotKey() {
        ...
        NSEvent.addLocalMonitorForEvents(matching: .keyDown) { event in
            if self.isRecording {
                ...
                print(
                    "Record hotkey - Key pressed: \(keyCode), modifiers: \(modifiers)"
                )

                // 检查是否有修饰键
                if modifiers.contains(.command) || modifiers.contains(.option) || modifiers.contains(.shift) || modifiers.contains(.control) {
                    self.hotKeyModifiers = UInt32(modifiers.rawValue)
                    self.hotKeyKeyCode = UInt32(keyCode)

                    print(
                        "Recorded hotkey: modifiers=\(self.hotKeyModifiers), keyCode=\(self.hotKeyKeyCode), hotkey=\(self.hotKeyToString())"
                    )
