...
class AppDelegate: NSObject, NSApplicationDelegate {
    ...
    @objc func toggleKeypad() {
        ...
        if isEnabled {
            ...
            enableEventTap()
            showNotification(title: "小键盘已开启", subtitle: "现在可以使用键盘上的特定键位输入数字")
        } else {
            print("Disabling keypad event tap")
            disableEventTap()
            showNotification(title: "小键盘已关闭", subtitle: "键盘已恢复正常模式")
        }
    ...
    func enableEventTap() {
        ...
        let eventTap = CGEvent.tapCreate(
            ...
            callback: { (proxy, type, event, refcon) -> Unmanaged<CGEvent>? in
                ...
                print("Keypad event tap - Key pressed: \(keyCode), flags: \(flags.rawValue)")
                ...
                if cmdPressed && optPressed && keyCode == Int32(appDelegate.hotKeyKeyCode) {
                    print(
                        "Hotkey pressed in keypad event tap, toggling keypad: \(appDelegate.isEnabled ? "off" : "on")"
                    )
                    DispatchQueue.main.async {
                        appDelegate.toggleKeypad()
                    }
    ...
    func mapKeyToNumber(keyCode: Int64, originalEvent: CGEvent) -> Unmanaged<CGEvent>? {
        ...
        if let mappedChar = keyMap[keyCode] {
            ...
            // 对于"00"这样的多字符映射，我们需要特殊处理
            if mappedChar == "00" {
                // 模拟按下0键两次
                for _ in 1...2 {
                    let zeroKeyCode = charToKeyCode("0")
                    let eventSource = CGEventSource(stateID: .hidSystemState)
                    let keyDownEvent = CGEvent(
                        keyboardEventSource: eventSource, virtualKey: zeroKeyCode, keyDown: true)
                    let keyUpEvent = CGEvent(
                        keyboardEventSource: eventSource, virtualKey: zeroKeyCode, keyDown: false)
