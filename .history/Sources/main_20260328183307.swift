    ...
    @objc func recordHotKey() {
        ...
        recordingEventMonitor = NSEvent.addLocalMonitorForEvents(matching: .keyDown) { [weak self] event in
            ...
            if self.isRecording {
                ...
                // 使用 NSEvent.ModifierFlags 检查修饰键
                ...
                // 构建修饰键值 - 使用与 CGEvent 检测相同的逻辑
                var modifiersValue = UInt32(0)
                
                // 关键：使用 NSEvent 的修饰键来构建 Carbon 常量
                // NSEvent.ModifierFlags.command 对应 cmdKey (1 << 8 = 256)
                // NSEvent.ModifierFlags.option 对应 optionKey (1 << 9 = 512)
                // NSEvent.ModifierFlags.shift 对应 shiftKey (1 << 10 = 1024)
                // NSEvent.ModifierFlags.control 对应 controlKey (1 << 11 = 2048)
                ...
                print(
                    "Built modifiersValue: \(modifiersValue)"
                )
