...
class AppDelegate: NSObject, NSApplicationDelegate {
    ...
    var recordRunLoopSource: CFRunLoopSource?
    ...
    @objc func recordHotKey() {
        ...
        // 先禁用之前可能存在的事件监听器
        if let eventTap = recordEventTap {
            CGEvent.tapEnable(tap: eventTap, enable: false)
            recordEventTap = nil
            print("Previous record hotkey event tap disabled")
        }
        ...
        print("Creating record hotkey event tap...")
        let eventTap = CGEvent.tapCreate(
            tap: .cgSessionEventTap,
            place: .headInsertEventTap,
            options: options,
        ...
        if let eventTap = eventTap {
            let runLoopSource = CFMachPortCreateRunLoopSource(kCFAllocatorDefault, eventTap, 0)
            self.recordRunLoopSource = runLoopSource
            CFRunLoopAddSource(CFRunLoopGetCurrent(), runLoopSource, .commonModes)
            CGEvent.tapEnable(tap: eventTap, enable: true)
            print("Record hotkey event tap enabled successfully")

            // 设置5秒超时
            DispatchQueue.main.asyncAfter(deadline: .now() + 5) {
                print("Recording timeout, stopping recording")
                self.stopRecording()
            }
        } else {
            print(
                "Failed to create record hotkey event tap - This usually means you need to grant accessibility permissions"
            )
            print(
                "Please go to System Settings > Privacy & Security > Accessibility and add Numaric to the list"
            )
    ...
    func stopRecording() {
        ...
        // 从运行循环中移除运行循环源
        if let runLoopSource = recordRunLoopSource {
            CFRunLoopRemoveSource(CFRunLoopGetCurrent(), runLoopSource, .commonModes)
            print("Record hotkey run loop source removed")
            recordRunLoopSource = nil
        }
