...
class AppDelegate: NSObject, NSApplicationDelegate {
    ...
    func setupEventTap() {
        // 设置默认快捷键为 Cmd+Opt+K
        hotKeyModifiers = (1 << kEventKeyModifierCommandMask) | (1 << kEventKeyModifierOptionMask)
        ...
        let eventTap = CGEvent.tapCreate(
            ...
            callback: { (proxy, type, event, refcon) -> Unmanaged<CGEvent>? in
                ...
                if cmdPressed { modifiers |= (1 << kEventKeyModifierCommandMask) }
                if optPressed { modifiers |= (1 << kEventKeyModifierOptionMask) }
                if shiftPressed { modifiers |= (1 << kEventKeyModifierShiftMask) }
                if controlPressed { modifiers |= (1 << kEventKeyModifierControlMask) }
        ...
        let eventTap = CGEvent.tapCreate(
            ...
            callback: { (proxy, type, event, refcon) -> Unmanaged<CGEvent>? in
                ...
                if modifiers > 0 {
                    ...
                    print("Recorded hotkey: modifiers=\(modifiers), keyCode=\(keyCode)")
    ...
    func hotKeyToString() -> String {
        ...
        if hotKeyModifiers & (1 << kEventKeyModifierCommandMask) != 0 {
            parts.append("⌘")
        }
        if hotKeyModifiers & (1 << kEventKeyModifierOptionMask) != 0 {
            parts.append("⌥")
        }
        if hotKeyModifiers & (1 << kEventKeyModifierShiftMask) != 0 {
            parts.append("⇧")
        }
        if hotKeyModifiers & (1 << kEventKeyModifierControlMask) != 0 {
            parts.append("⌃")
        }
    ...
    func loadSettings() {
        ...
        if let modifiers = defaults.object(forKey: "hotKeyModifiers") as? UInt32 {
            ...
            // 设置默认快捷键为 Cmd+Opt+K
            hotKeyModifiers =
                (1 << kEventKeyModifierCommandMask) | (1 << kEventKeyModifierOptionMask)
        }
